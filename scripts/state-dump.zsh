...
#                Compare like with like: a default-mode dump includes everything
#                ~/.zshrc does (completions, PATH additions), an --init dump only
#                what chitin loads.
...
command -v script >/dev/null || { print -u2 "state-dump.zsh needs script(1) to allocate a pseudo-terminal"; exit 1 }

# Bypass the ~/.zshenv lite-mode guard (non-TTY stdin, VSCODE_PID, or TERM_PROGRAM in
# vscode/cursor/zed) and drop inherited chitin state. TERM_PROGRAM is set to the same
# value for every dump, so it cannot be a source of differences between two dumps.
typeset -a clean_env
...
# The dump itself runs inside the shell (dump.zsh below), where CHI_DIR names the
# checkout that was actually loaded, whichever init.sh ~/.zshrc or --init pointed at.
cat > "$zdotdir/dump.zsh" <<'DUMP'
{
...
# Replace the loaded checkout's path with @ROOT@ only where it ends a path component
# or a value, so a sibling fiber such as <CHI_DIR>-cloud is left alone. In a zsh
# pattern a parameter expansion is literal unless written as ${~var}, so the path
# needs no escaping.
typeset dump="$(<"$STATE_DUMP_OUT.raw")"
dump="${dump//$CHI_DIR\//@ROOT@/}"
dump="${dump//$CHI_DIR\"/@ROOT@\"}"
dump="${dump//$CHI_DIR$'\n'/@ROOT@$'\n'}"
[[ "$dump" == *"$CHI_DIR" ]] && dump="${dump%$CHI_DIR}@ROOT@"
print -r -- "$dump" > "$STATE_DUMP_OUT"
rm -f -- "$STATE_DUMP_OUT.raw"
DUMP
print -r -- "source ${(qq)zdotdir}/dump.zsh" >> "$zdotdir/.zshrc"

rm -f -- "$out"
typeset shell_output
shell_output=$("${clean_env[@]}" "${extra_env[@]}" ZDOTDIR="$zdotdir" STATE_DUMP_OUT="$out" script -q /dev/null zsh -ic exit </dev/null 2>&1 | tr -d '\r')
typeset -i shell_status=$pipestatus[1]
# chitin's own log lines are informational (which path it took, how long); nothing else is shown
print -r -- "$shell_output" | sed -E 's/\x1b\[[0-9;]*m//g; s/\x1b\]1337;[^\x07\x1b]*//g' | grep -a 'chitin:' | grep -v 'initializing chitin' || true
if (( shell_status != 0 )) || [[ ! -s $out ]] || ! grep -q '^---FPATH---$' "$out"; then
  print -u2 "state-dump.zsh: the shell did not produce a complete dump (exit $shell_status). Its output was:"
  print -u2 -r -- "$shell_output" | tail -20 >&2
  exit 1
fi
